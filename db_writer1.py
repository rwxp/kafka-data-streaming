...
# Esta funcion evalua cada cierto tiempo si hay un nuevo evento. En caso de que exista un evento, va a guardar la informacion en base de datos.
##
##
...
def main(topic):
    ...
    my_schema = schema_registry_client.get_latest_version(
        topic+'-value').schema.schema_str
    ...
    while True:
        try:
            ...
            bid = json_deserializer(msg.value(), SerializationContext(
            ...
            try:
                ...
                print('seconds spent from web page to table:', time.time() -
                      time.mktime(time.strptime(bid_ts, '%Y-%m-%d %H:%M:%S')))
                print('-------------------------------')
